...
놓는 순간 저장소 v1의 다섯 모델이 전부 **183 %~626 %**, 즉 전일값 하나만 쓰는 예측기보다
못한 것으로 판명된다. 반대로 논문이 권고하는 모델 라인업과 「이전 시점 메탄이 1위 피처」라는
결론은 **논문의 예측 지평(≤24 h)에서만 성립**하므로, 주 단위 의사결정을 목표로 하는 우리
지평(h≥7일)에 그대로 옮기면 안 된다.
...
| HRT | 22~97일 **변동**(B), 23~140일(C) | 투입 일정 → 거의 상수 | ❌ §4.1 |
...
그대로 싣는다.** 이 규약을 저장소 v1 결과에 적용하면 다음과 같다(2023 홀드아웃, n=154,
같은 구간 naive RMSE = **383.5 ㎥/d**, gap=1 부분집합 n=103 에서 **269.5 ㎥/d**).

| v1 모델 | 보고된 R² | 보고된 RMSE | **RMSSE** | **RMSSE (gap=1)** | 논문 기준 판정 |
|---|:---:|:---:|:---:|:---:|---|
| RandomForest | 0.588 | 701.7 | **183 %** | 260 % | 기각 |
| MLP | −3.818 | 2,400.2 | **626 %** | 891 % | 기각 |
| LSTM | 0.170 | 996.0 | **260 %** | 370 % | 기각 |
| Transformer | 0.400 | 847.2 | **221 %** | 314 % | 기각 |
| Ensemble (NNLS) | 0.538 | 742.9 | **194 %** | 276 % | 기각 |
| *naive (persistence)* | *0.877* | *383.5* | *100 %* | *100 %* | — |

**R² 0.588 은 "쓸 만한 모델"이 아니라 "베이스라인의 절반 성능"이었다.** 이는 `MODELING_OVERVIEW`
...
**단, 이 피처가 쓸 수 있는 지평은 논문의 OD(≤24 h)뿐이다** — §4.2.
...
따라서 영천에서 HRT 열은 **분산이 거의 없는 열 + 임의 상수의 곱**이다. 논문의 FI 순위를 근거로
HRT 를 중요 피처로 채택하는 것은 잘못이다. (본 저장소 벤치마크는 HRT·OLR 을 계산해 넣되
순열중요도로 **영천에서 실제로 기여하는지**를 따로 판정한다 — §7.)
...
- 결과 → `outputs/benchmark_meola2025.json`
...
| ★3 | h=7·14일 지평 벤치마크 추가 | §4.2 — 운영상 실제 지평이며 persistence 가 무너지기 시작하는 구간(R² 0.566→0.252) |
| 4 | GA 데이터준비 최적화를 **폴드 내부로 닫아** 시험 도입 | §4.4 — 선택편향 27 ㎥/d 를 넘는 이득이 나오는지만 확인 |
| 5 | 저해 국면 데이터 확보 시 VFA 피처 재평가 | §3.7 — 논문은 저해 조건에서만 VFA 가 상위가 된다고 보고. 영천은 안정계라 현재 판정은 잠정 |
...
RMSSE 로 다시 재면 저장소 v1은 전부 기각이고(183~626 %), 논문이 권고하는 RF·RNN 은
영천의 지평(h≥7일)과 해상도(일별)에서는 근거가 약하며, 정작 논문이 가장 강하게 지지하는
것은 「모델이 아니라 입력과 계측을 고쳐라」는 우리 자신의 교훈(A1·B6)과 계량 개선 우선순위다.
