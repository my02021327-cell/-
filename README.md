...
> 📘 **모델링 설계 개요 v2 (공정 이해·전처리·결측 전략·모델 방향)** :
> **[docs/MODELING_OVERVIEW.md](docs/MODELING_OVERVIEW.md)**
>
> ⚠️ **v2 주요 정정** — 시설 참조문서(`docs/reference/`)·정제 데이터셋 반영 결과:
> ① 공정은 **2상(산발효조+병렬 2기 4,000㎥ CSTR)**, HRT 40.5일·OLR 1.14(부하 여유).
> ② 2023 "국면이동"은 **데이터 결함**(온도 센서 표류, ALK_B 오염, 9/18 이후 공백)이었음.
> ③ **Persistence(전일값) 베이스라인 R²=0.877**이 아래 결과표의 모든 모델을 상회 —
> 절대량 예측을 **Δ(차분)/잔차 예측으로 전환**한다.
> ④ 실질 병목은 **유리암모니아(FAN) 저해**이며 VFA/알칼리도 신호등은 이 실패 모드를
> 탐지하지 못한다(분모 상승 착시) — 관제는 다층 규칙(FAN·잔차 관리도 포함)으로 재설계.
> 아래 본문 결과표는 v1 기록으로 보존한다.
...
반응으로, 투입 유기물(VS)이 **한 번에 전환되지 않고 체류시간에 걸쳐 분산 전환** 된다.
따라서 오늘의 메탄생성량은 ① 과거 수일~수십일 **누적 투입 기질부하** 와 ② 현재 소화조
**내부 미생물 상태(VFA·알칼리도·pH·온도·VS)** 의 함수이다.

- **체류시간 응답 정량 확인** (`src/eda.py`): 투입 VS부하와 메탄의 교차상관에서 순간값
  r=0.54 대비 **10일 누적(이동평균) r=0.64** 로 더 강함 → 체류시간에 따른 분산 응답이 실재.
  이를 반영해 체류창 부하 피처(`VS_in_load5/10` 등)를 별도 설계.
  단, 시차 상관은 **lag 0에서 최대이고 단조 감소**하므로 응답은 사표시간을 갖는 '지연'이
  아니라 **완전혼합조(CSTR)의 지수감쇠 커널(적합 τ≈5일, r=0.639)** 이다 →
  자세한 동정 결과와 설계 근거는 **[docs/MODELING_OVERVIEW.md](docs/MODELING_OVERVIEW.md)**.
