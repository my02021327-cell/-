...
- **① 메탄생성량 14일 예측** : 현재 이화학 상태 + **투입 계획**을 반영한 다단계 예측(80% 구간).
  차트에 **horizon 신뢰도 구역(1–3 / 4–7 / 8–14일)** 을 음영으로 표시해 "며칠까지 믿을 수
  있는지"가 화면에서 바로 읽힌다.
- **② 상태이상 수준** : 11개 계측값의 설정값 이탈 → 4단계 신호등 + 최근 30일 신호 추이
- **③ 공정제어 지시** : 이탈 지표별 우선순위 조치
- 상시 만성 리스크(유리암모니아) 배너, 모델 성능 각주 포함
...
- **8–14일** R² 0.69~0.75 — 투입계획을 반영해야 유지(미지 시 0.50) → 기질수급·정비 계획용
- **15–28일** R² 0.65(투입계획 반영 시) — 투입 미지 시 0.19로 붕괴 → **시나리오 평가 전용**
- ⇒ **대시보드 기본 표시 14일**. persistence 는 7일 이후 급락하는 반면 모델은 완만히 유지되어
  **horizon 이 길수록 모델의 상대 우위가 커진다**(28일 R² 격차 0.96).
- 투입량은 **운전자가 계획하는 제어입력**이므로 계획값을 넣어 "이 투입계획이면 이 수준"을
  평가하는 시나리오 도구로 쓴다(Choi et al. 2025, *Water Research* 의 미래 투입부하 제공 전략과 동일).
