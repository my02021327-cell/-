...
<h2>K. 반응조 관성 — 무시하지도, 기대지도 않는다</h2>
<p>{R['관성보정']['형태']}</p>
<div class="note"><b>규칙 1(`y_lag1` 금지)과 충돌하지 않는다.</b>
{R['관성보정']['규칙1과의_관계']} τ 는 자유 파라미터가 아니라 물리적 시간상수이며,
상한은 {R['관성보정']['τ_물리상한']}</div>
{_tbl([("지평_일","지평 (일)",False),("n","n",True),("persistence_RMSE","persistence",True),
       ("기질모델_RMSE","기질 모델",True),("보정모델_RMSE","관성 보정",True),
       ("관성_이득_pct","관성 이득 %",True)], R["관성보정"]["지평별"])}
<p>전체 평균으로는 {R['관성보정']['전체']['관성보정_RMSE']} vs {R['관성보정']['전체']['기질만_RMSE']}
(Δ{R['관성보정']['전체']['ΔRMSE']}, p={R['관성보정']['전체']['p']}, {R['관성보정']['전체']['판정']}).
{R['관성보정']['해석']}</p>

<h2>L. 결측 처리 — 라벨을 만들어내지 않는다</h2>
<p><b>{R['결측처리']['진단']}</b> CH₄ = 유량 × 농도이므로 타깃 결측의 원인은 하나뿐이고,
그 하나가 매우 매끄러운 상태 변수다 — ACF(1)={R['결측처리']['CH4pct_평활도']['ACF1']},
일간 변화 SD {R['결측처리']['CH4pct_평활도']['일간변화_SD_pp']}%p,
결측 간격 중앙값 {R['결측처리']['간격분포']['중앙값_일']}일
({R['결측처리']['간격분포']['3일이하_pct']}%가 3일 이하).</p>
<div class="note crit-note"><b>"주말 CH₄% 가 낮다"는 관측은 근거가 없다.</b>
{R['결측처리']['주말차이_검증']}</div>
{_tbl([("항목","항목",False),("값","값",False)],
  [{"항목":"가림 실험 보간 오차 (간격 1일 / 3일)",
    "값":f"MAE {R['결측처리']['보간오차_가림실험']['간격1일_MAE_pp']} / {R['결측처리']['보간오차_가림실험']['간격3일_MAE_pp']} %p → 메탄 {R['결측처리']['보간오차_가림실험']['메탄환산_m3d']} ㎥/d"},
   {"항목":"보고용 계열 관측일","값":f"{R['결측처리']['결과']['보고용_계열_관측일']:,} / 2,086일"},
   {"항목":"  그중 실측","값":f"{R['결측처리']['결과']['그중_실측']:,}일 (성능 평가는 이 날만 사용)"},
   {"항목":"  재구성 (3일 이하 간격)","값":f"{R['결측처리']['결과']['재구성']:,}일, 불확실도 ±{R['결측처리']['결과']['재구성_평균불확실도_m3d']:.0f} ㎥/d 전파"},
   {"항목":"  남은 결측 (긴 간격)","값":f"{R['결측처리']['결과']['남은결측']:,}일 — NaN 유지"}])}
<p>정책:</p>
<ul>{"".join(f"<li>{x}</li>" for x in R["결측처리"]["정책"])}</ul>

<h2>M. 개선안 검증 — 채택과 기각</h2>
<p>{R['개선안_검증']['설명']}</p>
{sec_tbl("채택",
